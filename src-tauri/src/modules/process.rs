            ...
            for pid_u32 in &pids {
                ...
                if let Some(process) = system.process(pid) {
                                ...
                                (m_path_str.find(".app"), p_path_str.find(".app"))
                            {
                                if m_path_str[..m_idx + 4] == p_path_str[..p_idx + 4] {
                                    ...
                                    if !is_helper_by_args && !is_helper_by_name {
                                        ...
                                        crate::modules::logger::log_info("   => Identified as main process (manual path match)");
                                        break;
                                    }
                                }
                            }
                    ...
                    if !is_helper_by_name && !is_helper_by_args {
                        if main_pid.is_none() {
                            main_pid = Some(pid_u32);
                            crate::modules::logger::log_info("   => Identified as main process (Name/Args analysis)");
                        }
                    } else {
                        crate::modules::logger::log_info("   => Identified as helper process (Helper/Args)");
                    }
                }
            }
